...
привести к негативных последствиям (особенно на мобильных устройствах). Использовать 
их нужно с умом и следует предварительно убедиться, что аппаратное 
...
Кроме того, некоторые свойства могут привести к созданию **блока-обертки** 
...
свойство ведёт к созданию блока-обертки, указание его в качестве значения 
для `will-change` также ведёт к генерации блока-обертки для элементов с 
...
кроме создания стекового контекста и блока-обертки в ситуациях, описанных 
выше.
...
лишь способ скрытого общения с браузером), так что свойство может 
...
Тем не менее, наведение на родительский элемент не всегда является гарантией 
...
подсказать браузеру изменения, которые произойдут с элементом через 
...
отрисовки плавной длительной и/или быстрой прокрутки.
...
оптимизированный и высокопроизводительный код без применения хаков, 
и наделять производительность определенных CSS-операции большей значимостью.
